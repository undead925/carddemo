                    ...
                    if(sys_second <= between_time){
                        if(game_no == "第三场"){
                            if(rain_stop){
                                ...
                                layer.open({
                                    type: 1,
                                    content: '<div class="layer-result over"><h2 class="layer-result-title">活动已结束，<br>更多精彩正在继续！</h2><div class="layer-result-pic"><img src="/assets/dist/topic/reward_rain/images/over_bg.png" class="w-100"></div><a href="'+url+'" class="btn-result">去看看</a></div>',
                ...
                $.ajax({
                    ...
                    success:function(json){
                        if(json.status == 0){
                            ...
                            }else if(json.code == 12000){
                                layer.open({
                                    content: '亲排队人太多！请您稍后再试！',
                                    btn: ['回到首页'],
                                    shadeClose: false,
                                    yes: function () {
                                        if(device_port != "ios" && device_port != "android"){
            ...
            var rain = setInterval(function() {
                ...
                if ( temp_time >= 30000) {
                    if(red_envelope > 0){
                        ...
                        layer.open({
                            type: 1,
                            content: '<div class="layer-result"><h2 class="layer-result-title">恭喜您本场获得了</h2><div class="layer-result-pic"><img src="/assets/dist/topic/reward_rain/images/hongbao_bg.png" class="hongbao"><ul><li>红包: '+type_A+'个</li><li>红包券: '+type_B+'张</li></ul></div><a href="'+url+'" class="btn-result">查看战果</a></div>',
                            style: 'width: 90%; background-color: transparent;',
                            shadeClose: false
                        });
                    }else{
                        if(game_no == "第一场"){
                            layer.open({
                                type: 1,
                                content: '<div class="layer-result"><h2 class="layer-result-title">手太慢，没有抓住有奖红包！</h2><div class="layer-result-tips">下场活动<span class="c-red">15:00</span>开始，记得来哦！</div><div class="layer-result-pic"><img src="/assets/dist/topic/reward_rain/images/miss_bg.png" class="w-100"></div><a href="###" class="btn-result">回到会场</a></div>',
                                style: 'width: 90%; background-color: transparent;',
                                shadeClose: false
                            });
                            ...
                            layer.open({
                                type: 1,
                                content: '<div class="layer-result"><h2 class="layer-result-title">真是可惜呀，换个姿势再抓！</h2><div class="layer-result-tips">下场活动<span class="c-red">20:00</span>开始，记得来哦！</div><div class="layer-result-pic"><img src="/assets/dist/topic/reward_rain/images/miss_bg.png" class="w-100"></div><a href="###" class="btn-result">回到会场</a></div>',
                                style: 'width: 90%; background-color: transparent;',
                                shadeClose: false
                            });
                            $(".btn-result").bind("click", function(){
                    ...
                    if (left >= 50) {
                        html += "right:" + (100 - left) + "%;background: url(/assets/dist/topic/reward_rain/images/"+pic_arr[pic_index]+") no-repeat center top;background-size: 100%;' data='"+boom_pic_arr[pic_index]+"'></div>";
                    } else {
                        html += "left:" + left + "%;background: url(/assets/dist/topic/reward_rain/images/"+pic_arr[pic_index]+") no-repeat center top;background-size: 100%;' data='"+boom_pic_arr[pic_index]+"'></div>";
                    }
                    $(".game").append(html);
                    $("#" + counter).bind("touchstart", function() {
                        ...
                        $(this).stop();
                        $(this).css("background-image", "url(/assets/dist/topic/reward_rain/images/"+$(this).attr("data")+")");
                        $(this).css("opacity", 1);
                        setTimeout(function() {
                            $("#" + id).remove();
                        }, 1500);
